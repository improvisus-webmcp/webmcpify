...
const tool = { id: "run_action", name: "run_action", description: "Runs the discovered action.", parameters: { type: "object", properties: {}, required: [] }, implementation: { handler: "src/App.tsx#run", action: "run the discovered action" }, placement: { strategy: "imperative", file: "src/App.tsx", rationale: "Keeps registration beside the action." }, sourceFiles: ["src/App.tsx"] };
await writeProposedTools(sitePath, [tool], "discovery.json", "fixture-generation.json");
const generation = await createTrajectoryArtifact("generate", "TOOL_PROPOSALS_JSON fixture", { sitePath, provider: "fixture", fixture: true });
...
const response = await fetch("http://127.0.0.1:4387/approve", { method: "POST", headers: { "content-type": "application/x-www-form-urlencoded" }, body: new URLSearchParams({ toolIds: tool.id, toolsJson: JSON.stringify({ tools: [editedTool] }), taskIds: tasks.map((task) => task.id), tasksJson: JSON.stringify(tasks), approveSourceDiff: "yes" }) });
